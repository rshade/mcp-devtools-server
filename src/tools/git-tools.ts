...
const GitDiffArgsSchema = z.object({
  ...
  base: z
    .string()
    .optional()
    .describe("Base branch or commit to compare against"),
...
const GitLogArgsSchema = z.object({
  ...
  since: z
    .string()
    .optional()
    .describe('Show commits since date (e.g., "2 weeks ago")'),
...
const CodeReviewArgsSchema = z.object({
  ...
  maxFiles: z
    .number()
    .min(1)
    .max(MAX_FILES_TO_REVIEW)
    .optional()
    .describe(`Maximum number of files to review (1-${MAX_FILES_TO_REVIEW})`),
  focus: z
    .enum(["security", "performance", "maintainability", "all"])
    .optional()
    .describe("Focus area for review"),
});
...
const PRMessageArgsSchema = z.object({
  ...
  includeIssue: z
    .string()
    .optional()
    .describe('Issue number to reference (e.g., "123")'),
  useTemplate: z
    .boolean()
    .optional()
    .describe("Use GitHub PR template if available (default: true)"),
  maxCommits: z
    .number()
    .min(1)
    .max(200)
    .optional()
    .describe(
      `Maximum number of commits to analyze (default: ${DEFAULT_PR_LOG_COUNT})`,
    ),
});
...
export class GitTools {
  ...
  async gitDiff(args: GitDiffArgs): Promise<GitToolResult> {
    ...
    const cached = this.cacheManager.get<GitToolResult>(
      "gitOperations",
      cacheKey,
    );
    if (cached) {
      ...
      if (!focus || focus === "performance" || focus === "all") {
        ...
        if (forCount + whileCount >= NESTED_LOOP_THRESHOLD) {
          concerns.push({
            ...
            category: "performance",
            message:
              "Multiple nested loops detected - potential performance concern",
          });
          concernCount++;
        }
      }
  ...
  private generateBody(options: {
    ...
    if (options.changes.length > 1) {
      body += `## Changes\n\n`;
      const uniqueChanges = [...new Set(options.changes)].slice(
        0,
  ...
  private mergeWithTemplate(
    ...
    const replacements: Record<string, string> = {
      ...
      // Issue reference
      "<!-- Related Issues -->": options.issueNumber
    ...
    if (
      options.issueNumber &&
      !body.includes(`Fixes #${options.issueNumber}`)
    ) {
      body += `\n\nFixes #${options.issueNumber}\n`;
    }
  ...
  private formatChanges(changes: string[]): string {
    ...
    let output = "";
    const uniqueChanges = [...new Set(changes)].slice(
      0,
      MAX_CHANGES_TO_DISPLAY,
    );
    for (const change of uniqueChanges) {
