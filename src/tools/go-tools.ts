...
const GoToolArgsSchema = z.object({
  ...
  package: z
    .string()
    .optional()
    .describe("Go package to operate on (e.g., ./...)"),
...
const GoLintArgsSchema = z.object({
  ...
  enabledLinters: z
    .array(z.string())
    .optional()
    .describe("Specific linters to enable"),
  disabledLinters: z
    .array(z.string())
    .optional()
    .describe("Specific linters to disable"),
  verbose: z.boolean().optional().describe("Enable verbose output"),
  format: z
    .string()
    .optional()
  ...
  concurrency: z
    .number()
    .optional()
    .describe("Number of CPUs to use for linting"),
  timeout: z.number().optional().describe("Timeout for linting in seconds"),
  paths: z
    .array(z.string())
    .optional()
    .describe("Specific paths/packages to lint"),
});
...
const GoBenchmarkArgsSchema = z.object({
  ...
  package: z
    .string()
    .optional()
    .describe("Go package to benchmark (e.g., ./...)"),
  benchmarks: z
    .string()
    .optional()
    .describe("Benchmark pattern to run (e.g., BenchmarkFoo)"),
  benchtime: z.string().optional().describe("Benchmark time (e.g., 10s, 100x)"),
  benchmem: z
    .boolean()
    .optional()
    .describe("Print memory allocation statistics"),
  cpu: z
    .array(z.number())
    .optional()
    .describe("CPU counts to test (e.g., [1, 2, 4])"),
...
const GoBuildArgsSchema = z.object({
  ...
  goos: z
    .string()
    .optional()
    .describe("Target operating system (linux, darwin, windows, etc.)"),
  goarch: z
    .string()
    .optional()
    .describe("Target architecture (amd64, arm64, 386, etc.)"),
  tags: z.array(z.string()).optional().describe("Build tags to include"),
  verbose: z.boolean().optional().describe("Enable verbose output"),
  race: z.boolean().optional().describe("Enable race condition detection"),
  args: z.array(z.string()).optional().describe("Additional arguments"),
...
const GoGenerateArgsSchema = z.object({
  ...
  package: z
    .string()
    .optional()
    .describe("Go package to run generate on (e.g., ./...)"),
  run: z
    .string()
    .optional()
    .describe("Only run generate directives matching this regex"),
  skip: z
    .string()
    .optional()
    .describe("Skip generate directives matching this regex"),
  verbose: z.boolean().optional().describe("Enable verbose output"),
  dryRun: z
    .boolean()
    .optional()
    .describe("Print commands without running them"),
  args: z.array(z.string()).optional().describe("Additional arguments"),
  timeout: z.number().optional().describe("Command timeout in milliseconds"),
});
...
const GoWorkArgsSchema = z.object({
  ...
  command: z
    ...
    .describe("Workspace command to run"),
  modules: z
    .array(z.string())
    .optional()
    .describe("Module paths for use command"),
  args: z.array(z.string()).optional().describe("Additional arguments"),
});
...
const GoVulncheckArgsSchema = z.object({
  ...
  mode: z
    .enum(["source", "binary"])
    .optional()
    .describe("Analysis mode (source or binary)"),
  ...
  async getProjectInfo(directory?: string): Promise<GoProjectInfo> {
    ...
    try {
      ...
      try {
        ...
        if (useMatches) {
          info.workspaces = useMatches.map((match) =>
            match.replace(/^use\s+/, "").trim(),
          );
        }
      } catch {
        // No go.work file
