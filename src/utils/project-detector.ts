...
export class ProjectDetector {
  ...
  async detectProject(): Promise<ProjectInfo> {
    ...
    const cached = this.cacheManager.get<ProjectInfo>(
      "projectDetection",
      cacheKey,
    );
    if (cached) {
  ...
  private detectBuildSystem(configFiles: ConfigFile[]): BuildSystem {
    ...
    if (packageFile) {
      if (configFiles.some((cf) => cf.name === "pnpm-lock.yaml"))
        return BuildSystem.PNPM;
      if (configFiles.some((cf) => cf.name === "yarn.lock"))
    ...
    if (
      configFiles.some(
        (cf) => cf.name === "pyproject.toml" && cf.type === "package",
      )
    ) {
    ...
    if (
      configFiles.some(
        (cf) => cf.name.includes(".csproj") || cf.name.includes(".sln"),
      )
    ) {
