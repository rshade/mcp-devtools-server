...
describe('FileValidationTools', () => {
  ...
  describe('file filtering', () => {
    it('should skip binary files when skipBinary=true', async () => {
      ...
      expect(result.filesSkipped.some(f => f.reason === 'binary file')).toBe(true);
    });
    ...
    it('should respect exclude patterns', async () => {
      ...
      expect(result.filesWithoutNewline.every(f => !f.includes('edge-cases'))).toBe(true);
    });
  ...
  describe('summary generation', () => {
    it('should generate accurate summaries for mixed results', async () => {
      ...
      const sourcePath2 = path.join(fixturesDir, 'text', 'lf-without-newline.txt');
