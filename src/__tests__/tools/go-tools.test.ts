...
describe('GoTools - P1 Advanced Features', () => {
  ...
  describe('Schema Validation - Work', () => {
    ...
    it('should handle work with modules', () => {
      const validated = GoTools.validateWorkArgs({ command: 'use', modules: ['./mod'] });
  ...
  describe('Schema Validation - Vulncheck', () => {
    ...
    it('should accept both modes', () => {
      expect(() => GoTools.validateVulncheckArgs({ mode: 'source' })).not.toThrow();
      expect(() => GoTools.validateVulncheckArgs({ mode: 'binary' })).not.toThrow();
    });
  });
  ...
  describe('goBenchmark', () => {
    ...
    it('should handle benchtime parameter', async () => {
      mockExecute.mockResolvedValue({ success: true, stdout: '', stderr: '', exitCode: 0, duration: 100 });
  ...
  describe('goGenerate', () => {
    it('should execute generate command', async () => {
      mockExecute.mockResolvedValue({ success: true, stdout: 'generating...', stderr: '', exitCode: 0, duration: 500 });
  ...
  describe('goBuild - Cross-Compilation', () => {
    it('should set GOOS environment variable', async () => {
      mockExecute.mockResolvedValue({ success: true, stdout: '', stderr: '', exitCode: 0, duration: 1000 });
  ...
  describe('goWork', () => {
    ...
    it('should execute work use with modules', async () => {
      mockExecute.mockResolvedValue({ success: true, stdout: '', stderr: '', exitCode: 0, duration: 100 });
      await tools.goWork({ command: 'use', modules: ['./moduleA', './moduleB'] });
      expect(mockExecute).toHaveBeenCalledWith('go', expect.objectContaining({
  ...
  describe('goVulncheck', () => {
    it('should execute vulncheck in source mode', async () => {
      ...
      expect(mockExecute).toHaveBeenCalledWith('govulncheck', expect.any(Object));
      expect(result.success).toBe(true);
    });
    ...
    it('should execute vulncheck in binary mode', async () => {
      ...
      expect(mockExecute).toHaveBeenCalledWith('govulncheck', expect.objectContaining({
        args: expect.arrayContaining(['./bin/app'])
      }));
    });

    it('should handle JSON output', async () => {
      mockExecute.mockResolvedValue({ success: true, stdout: '{}', stderr: '', exitCode: 0, duration: 1500 });
