...
describe("MakeTools", () => {
  ...
  describe("getProjectStatus", () => {
    it("should return status when Makefile exists", async () => {
      mockDetectProject.mockResolvedValue({
        ...
        configFiles: [{ type: "makefile", path: "/project/Makefile" }],
        makeTargets: ["build", "test", "lint", "clean"],
      });
      mockGetProjectContext.mockResolvedValue(
        "TypeScript project with Makefile",
      );
  ...
  describe("Error Handling and Suggestions", () => {
    ...
    it("should suggest available targets when target not found", async () => {
      ...
      expect(
        result.suggestions?.some((s) => s.includes("Available targets")),
      ).toBe(true);
    });
    ...
    it("should handle exceptions during execution", async () => {
      ...
      expect(result.suggestions).toContain(
        "Check if Makefile exists and target is defined",
      );
    });
  });
