...
describe('GitTools', () => {
  ...
  describe('Schema Validation', () => {
    ...
    describe('validateCodeReviewArgs', () => {
      ...
      it('should accept valid focus values', () => {
        const focuses = ['security', 'performance', 'maintainability', 'all'] as const;
      ...
      it('should enforce maxFiles range', () => {
        expect(() => GitTools.validateCodeReviewArgs({ maxFiles: 0 })).toThrow();
        expect(() => GitTools.validateCodeReviewArgs({ maxFiles: 101 })).toThrow();
        expect(() => GitTools.validateCodeReviewArgs({ maxFiles: 50 })).not.toThrow();
      });
    });
    ...
    describe('validatePRMessageArgs', () => {
      ...
      it('should accept all valid commit types', () => {
        const types = ['feat', 'fix', 'docs', 'style', 'refactor', 'perf', 'test', 'chore'] as const;
  ...
  describe('gitStatus', () => {
    ...
    it('should handle git errors', async () => {
      ...
      expect(result.suggestions).toContain('This directory is not a Git repository');
    });
  });
...
+const password = 'mypassword';
 export const config = {};`;
      ...
      const securityConcerns = result.concerns.filter(c => c.category === 'security');
...
+}}}`;
      ...
      const result = await tools.codeReview({ base: 'main', focus: 'performance' });

      expect(result.success).toBe(true);
      const perfConcerns = result.concerns.filter(c => c.category === 'performance');
...
+const veryLongLineThatExceedsTheMaximumLineLengthLimitAndShouldBeDetectedByTheMaintainabilityCheckBecauseItMakesCodeHardToReadAndShouldBeWrapped = true;`;
      ...
      const result = await tools.codeReview({ base: 'main', focus: 'maintainability' });

      expect(result.success).toBe(true);
      const maintConcerns = result.concerns.filter(c => c.category === 'maintainability');
...
+const password = 'test-password';`;
      ...
      const result = await tools.codeReview({ base: 'main', includeTests: false });
...
docs: update README`;
    ...
    it('should use specified commit type', async () => {
      ...
      const result = await tools.generatePRMessage({ base: 'main', type: 'fix' });
...
${diff}`;
        ...
        const result = await tools.codeReview({ base: 'main', focus: 'security' });
...
+exec(command);`;
  ...
  describe('Error Handling', () => {
    ...
    it('should provide helpful suggestions for unknown revision', async () => {
      ...
      expect(result.success).toBe(false);
      if (result.suggestions) {
        expect(result.suggestions).toContain('The specified branch or commit does not exist');
      }
    });
