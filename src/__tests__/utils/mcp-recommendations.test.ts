...
describe("MCPRecommendations", () => {
  ...
  describe("getRecommendationsByCategory", () => {
    it("should return AI-related MCP servers for AI category", () => {
      const aiRecs = recommendations.getRecommendationsByCategory(
        MCPCategory.AI,
      );

      expect(aiRecs.length).toBeGreaterThan(0);
      expect(aiRecs.every((r) => r.categories.includes(MCPCategory.AI))).toBe(
        true,
      );
    ...
    it("should return testing-related MCP servers for Testing category", () => {
      const testRecs = recommendations.getRecommendationsByCategory(
        MCPCategory.Testing,
      );

      expect(testRecs.length).toBeGreaterThan(0);
      expect(
        testRecs.every((r) => r.categories.includes(MCPCategory.Testing)),
      ).toBe(true);
      expect(testRecs.some((r) => r.name === "Playwright")).toBe(true);
    });

    it("should return database-related MCP servers for Database category", () => {
      const dbRecs = recommendations.getRecommendationsByCategory(
        MCPCategory.Database,
      );

      expect(dbRecs.length).toBeGreaterThan(0);
      expect(
        dbRecs.every((r) => r.categories.includes(MCPCategory.Database)),
      ).toBe(true);
    ...
    it("should return filesystem-related MCP servers for FileSystem category", () => {
      const fsRecs = recommendations.getRecommendationsByCategory(
        MCPCategory.FileSystem,
      );

      expect(fsRecs.length).toBeGreaterThan(0);
      expect(
        fsRecs.every((r) => r.categories.includes(MCPCategory.FileSystem)),
      ).toBe(true);
    ...
    it("should return development-related MCP servers for Development category", () => {
      const devRecs = recommendations.getRecommendationsByCategory(
        MCPCategory.Development,
      );

      expect(devRecs.length).toBeGreaterThan(0);
      expect(
        devRecs.every((r) => r.categories.includes(MCPCategory.Development)),
      ).toBe(true);
      expect(devRecs.some((r) => r.name === "Git")).toBe(true);
    });

    it("should return empty array for non-existent category", () => {
      const recs = recommendations.getRecommendationsByCategory(
        "NonExistent" as MCPCategory,
      );
  ...
  describe("getRecommendationsByPriority", () => {
    it("should return high priority recommendations", () => {
      const highPriorityRecs =
        recommendations.getRecommendationsByPriority("high");
      ...
      expect(
        highPriorityRecs.some((r) => r.name === "Sequential Thinking"),
      ).toBe(true);
    });

    it("should return medium priority recommendations", () => {
      const mediumPriorityRecs =
        recommendations.getRecommendationsByPriority("medium");

      expect(mediumPriorityRecs.length).toBeGreaterThan(0);
      expect(mediumPriorityRecs.every((r) => r.priority === "medium")).toBe(
        true,
      );
    });

    it("should return low priority recommendations", () => {
      const lowPriorityRecs =
        recommendations.getRecommendationsByPriority("low");
  ...
  describe("getRecommendationsForUseCase", () => {
    it("should return recommendations for browser testing use case", () => {
      const browserRecs =
        recommendations.getRecommendationsForUseCase("browser testing");
    ...
    it("should return recommendations for documentation use case", () => {
      const docRecs =
        recommendations.getRecommendationsForUseCase("documentation");
    ...
    it("should return recommendations for database use case", () => {
      ...
      expect(dbRecs.length).toBeGreaterThan(0);
      expect(
        dbRecs.some((r) => r.name === "PostgreSQL" || r.name === "SQLite"),
      ).toBe(true);
    });

    it("should return recommendations for problem solving use case", () => {
      const problemRecs =
        recommendations.getRecommendationsForUseCase("problem solving");

      expect(problemRecs.length).toBeGreaterThan(0);
      expect(problemRecs.some((r) => r.name === "Sequential Thinking")).toBe(
        true,
      );
    });
    ...
    it("should perform case-insensitive matching", () => {
      ...
      expect(upperRecs.map((r) => r.name).sort()).toEqual(
        lowerRecs.map((r) => r.name).sort(),
      );
    });

    it("should return empty array for non-matching use case", () => {
      const recs =
        recommendations.getRecommendationsForUseCase("quantum computing");
  ...
  describe("getContextualRecommendations", () => {
    it("should recommend database servers for projects with databases", () => {
      ...
      expect(
        recs.some((r) => r.name === "PostgreSQL" || r.name === "SQLite"),
      ).toBe(true);
    });
    ...
    it("should recommend testing servers for projects with tests", () => {
      ...
      expect(recs.some((r) => r.categories.includes(MCPCategory.Testing))).toBe(
        true,
      );
    });
    ...
    it("should handle projects with no special features", () => {
      ...
      expect(
        recs.some(
          (r) => r.name === "Context7" || r.name === "Sequential Thinking",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("generateMCPConfig", () => {
    it("should generate valid .mcp.json configuration for given recommendations", () => {
      ...
      expect(
        Object.keys((config as ConfigType).mcpServers).length,
      ).toBeGreaterThan(0);
    });
    ...
    it("should include command and args in config", () => {
      ...
      Object.values((config as ConfigType).mcpServers as ServerConfig).forEach(
  ...
  describe("recommendation content validation", () => {
    it("should have valid package names for all recommendations", () => {
      const allRecs = recommendations
        .getRecommendationsByPriority("high")
  ...
  describe("integration scenarios", () => {
    it("should provide comprehensive recommendations for full-stack web application", () => {
      ...
      expect(
        recs.some((r) => r.categories.includes(MCPCategory.Database)),
      ).toBe(true);
      expect(recs.some((r) => r.categories.includes(MCPCategory.Web))).toBe(
        true,
      );
    });
    ...
    it("should provide recommendations for CLI tool development", () => {
      ...
      expect(recs.length).toBeGreaterThan(0);
      expect(
        recs.some((r) => r.categories.includes(MCPCategory.Development)),
      ).toBe(true);
    });
