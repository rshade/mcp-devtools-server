...
import { ProjectType, ProjectDetector, BuildSystem } from '../../utils/project-detector.js';
...
describe('SuggestionEngine', () => {
  ...
  describe('generateSuggestions', () => {
    ...
    it('should use cache for identical failures', async () => {
      ...
      expect(suggestions2.suggestions.length).toBe(suggestions1.suggestions.length);
      expect(suggestions2.suggestions[0].title).toBe(suggestions1.suggestions[0].title);
    });
  });
  ...
  describe('analyzeHistory', () => {
    it('should detect low success rates', async () => {
      const results: ExecutionResult[] = [
        { command: 'test1', stdout: '', stderr: 'error', success: false, exitCode: 1, duration: 100 },
        { command: 'test2', stdout: '', stderr: 'error', success: false, exitCode: 1, duration: 100 },
        { command: 'test3', stdout: '', stderr: 'error', success: false, exitCode: 1, duration: 100 },
        { command: 'test4', stdout: 'ok', stderr: '', success: true, exitCode: 0, duration: 100 },
      ];

      const suggestions = await engine.analyzeHistory(results);

      expect(suggestions.length).toBeGreaterThan(0);
      expect(suggestions.some(s => s.title.includes('Low Success Rate'))).toBe(true);
    });

    it('should detect recurring issues', async () => {
      const results: ExecutionResult[] = [
        { command: 'test1', stdout: '', stderr: 'cannot find package "foo"', success: false, exitCode: 1, duration: 100 },
        { command: 'test2', stdout: '', stderr: 'cannot find package "foo"', success: false, exitCode: 1, duration: 100 },
        { command: 'test3', stdout: '', stderr: 'cannot find package "foo"', success: false, exitCode: 1, duration: 100 },
      ];
    ...
    it('should handle all successful commands', async () => {
      const results: ExecutionResult[] = [
        { command: 'test1', stdout: 'ok', stderr: '', success: true, exitCode: 0, duration: 100 },
        { command: 'test2', stdout: 'ok', stderr: '', success: true, exitCode: 0, duration: 100 },
        { command: 'test3', stdout: 'ok', stderr: '', success: true, exitCode: 0, duration: 100 },
      ];
