...
describe("KnowledgeBase", () => {
  ...
  describe("getPatternsByCategory", () => {
    it("should return patterns for security category", () => {
      ...
      expect(patterns.every((p) => p.category === Category.Security)).toBe(
        true,
      );
    });
    ...
    it("should return patterns for dependencies category", () => {
      ...
      expect(patterns.length).toBeGreaterThan(0);
      expect(patterns.every((p) => p.category === Category.Dependencies)).toBe(
        true,
      );
    });
